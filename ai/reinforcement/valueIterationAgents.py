...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for k in range(0, self.iterations):
            valueCollection = self.values.copy() # Do not change self.values on the fly
            ...
            for state in self.mdp.getStates():
                ...
                valueCollection[state] = maxQvalue
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for trans_state, prob in self.mdp.getTransitionStatesAndProbs(state,action):
                Qvalue += prob * (self.mdp.getReward(state, action, trans_state) + self.discount*self.values[trans_state])
        return Qvalue
